...
function [7:0] active_col_start;
    ...
    input [2:0] row;
    begin
        active_col_start = (value_only) ? VALUE_COL_START : row_col_start(row);
    end
endfunction
...
function [7:0] active_col_end;
    ...
    input [2:0] row;
    begin
        active_col_end = (value_only) ? VALUE_COL_END : row_col_end(row);
    end
endfunction
...
function [7:0] display_char;
    ...
    begin
        ...
        case (index)
            ...
            5'd7: display_char = digit_char(value, 3'd4);
...
always @(*) begin
    case (cur_state)
        ...
        I2C_INIT:          next_state = (i2c_done) ? INIT_SEND : I2C_INIT;
        INIT_SEND:         next_state = (i2c_done && init_index == 5'd16) ? CLEAR_COLUMN_ADDR : INIT_SEND;
        ...
        CLEAR_SCREEN:      next_state = (i2c_done && clear_index == 8'd127) ? SET_COLUMN_ADDR : CLEAR_SCREEN;
        SET_COLUMN_ADDR:   next_state = (i2c_done) ? SET_PAGE_ADDR : SET_COLUMN_ADDR;
        SET_PAGE_ADDR:     next_state = (i2c_done) ? WRITE_CHAR : SET_PAGE_ADDR;
        WRITE_CHAR:        next_state = (i2c_done && active_write_col == active_col_end(value_only_refresh, row_index)) ? ((row_index == ROW_LAST) ? WAIT_REFRESH : SET_COLUMN_ADDR) : WRITE_CHAR;
        WAIT_REFRESH:      next_state = (AUTO_REFRESH && refresh_cnt == REFRESH_DELAY) ? SET_COLUMN_ADDR : WAIT_REFRESH;
        ...
        case (cur_state)
            ...
            WRITE_CHAR: begin
                ...
                data <= (TEST_PATTERN) ? test_pattern_col(row_index, active_write_col) :
                                          bitmap_col(
                                              char_bitmap(display_char(row_index, active_write_col >> 3)),
                                              active_write_col[2:0]
                                          );
                char_index <= active_write_col >> 3;
                if (i2c_done && active_write_col < active_col_end(value_only_refresh, row_index))
                    col_index <= col_index + 1'b1;
                if (i2c_done && active_write_col == active_col_end(value_only_refresh, row_index)) begin
