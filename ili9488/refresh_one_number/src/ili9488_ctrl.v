...
assign digit_on = in_digit_box && digit_pixel_5x7(draw_digit_value, cell_x, cell_y);
    ...
    if (!sys_rst_n) begin
        ...
        if (delay_cnt != 27'd0) begin
            ...
            case (state)
                ...
                ST_HOLD: begin
                    if (digit_update) begin
                        digit_update_ack <= 1'b1;
                        seq_idx    <= 4'd0;
                        delay_cnt  <= TX_GAP_CYC;
                        next_state <= ST_CASET;
                        state      <= ST_DELAY;
